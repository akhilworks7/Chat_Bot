...
def render_usage_tab(user: dict):
    ...
    # Real-time synchronization of document & vector metrics
    docs_count = len(user_docs)
    storage_mb = round(sum([d.file_size for d in user_docs]) / (1024 * 1024), 2)
    vecs_count = live_vecs_count if live_vecs_count > 0 else sum([d.vector_count for d in user_docs])
    ...
    # Auto-synchronize live accurate metrics into DB usage statistics table
    with get_db() as db:
        u_stat = crud.get_or_create_usage_statistics(db, user_id)
        if u_stat.documents_uploaded != docs_count or u_stat.vector_count != vecs_count:
            u_stat.documents_uploaded = docs_count
            u_stat.vector_count = vecs_count
            u_stat.storage_used = sum([d.file_size for d in user_docs])
            db.flush()
